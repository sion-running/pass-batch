...
import com.fastcamplus.pass.repository.pass.*;
import com.fastcamplus.pass.repository.user.UserGroupMappingEntity;
import com.fastcamplus.pass.repository.user.UserGroupMappingRepository;
...
public class AddPassesTasklet implements Tasklet {
    ...
    private int addPasses(BulkPassEntity bulkPassEntity, List<String> userIds) {
        ...
        for (String userId : userIds) {
            PassEntity passEntity = PassModelMapper.INSTANCE.toPassEntity(bulkPassEntity, userId); // setter로 구현해도 됨
            // TODO mapstruct.Mapper 더 공부하려면, 예시 플젝에 PassModelMapperTest 코드 참고
            passEntities.add(passEntity);
        }

        List<PassEntity> saved = passRepository.saveAll(passEntities);
